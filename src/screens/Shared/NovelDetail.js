...
class NovelDetail extends Component {
  ...
  handleOnPressReport = () => {
    ...
    Linking.openURL( `https://www.wilddream.net/Art/index/report/typeid/2/contentid/${item.id}` )
  };

  handleOnPressShareNovel = () => {
    const { item } = this.props;
    const shareOptions = {
      message: `${item.title} | ${item.user.name} #WildDream\nhttps://www.wilddream.net/journal/view/${item.id}`
    };
        ...
        >
          ...
          {isActionButtonVisible && item && (
            <FloatingAction
              ...
              overlayColor="rgba(255,0,0,0.5)" // Maps to bgColor="red" from original
              floatingIcon={this.renderBookmarkButtonIcon()}
              showBackground={false}
              distanceToEdge={16}
              position="right"
              animated={false}
              onPressMain={() => {
                // The BookmarkNovelButton component likely handles its own press events
              }}
            />
          )}
          <DetailInfoModal
